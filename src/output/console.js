...
/**
 * Print a formatted table of projections to the console.
...
export function printResults(results) {
  ...
  for (const r of results) {
    ...
    // Color coding
    let pickColor = '';
    if (pick === 'OVER') pickColor = '\x1b[32m'; // green
    else if (pick === 'UNDER') pickColor = '\x1b[34m'; // blue
    else if (pick === 'NO PLAY') pickColor = '\x1b[90m'; // gray
    const reset = pickColor ? '\x1b[0m' : '';

    console.log(
      `${matchup}    ${model}  ${line}  ${edge.padStart(6)}  ${pickColor}${pick.padEnd(8)}${reset}  ${conf}`
    );
  }
